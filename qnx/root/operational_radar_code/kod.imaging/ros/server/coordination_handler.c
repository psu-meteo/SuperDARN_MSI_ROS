...
void *coordination_handler(struct ControlProgram *control_program)
{
   ...
   int gpssecond,gpsnsecond,gpscapture;
...
/*Calculate Ready State*/
   ...
   if (verbose > 1 ) fprintf(stderr,"Coord: Num ready: %d  Num clients: %d Ready State: %d  Trigger State: %d\n",numready,numcontrolprograms,ready_state,trigger_state);
      ...
      switch(ready_state) {
        ...
        /*all control programs ready for trigger*/
          ...
          if (verbose > 1) { 
              ...
              fprintf(stderr,"Coord: Pre-Trigger Active Check %d Elapsed Microseconds: %ld\n",i,elapsed);
          }
 ...
 */       
          ...
          if (verbose > 1) { 
            ...
            fprintf(stderr,"Coord: Trigger Elapsed Microseconds: %ld\n",elapsed);
          }
          ...
          recv_data(gpssock,&gpscapture, sizeof(int));
...
//              pthread_join(threads[i],NULL);
          ...
          if (verbose > 1) { 
            ...
            fprintf(stderr,"Coord: Post Trigger Elapsed Microseconds: %ld\n",elapsed);
          }
          if (verbose > 1) { 
            elapsed=(t6.tv_sec-t0.tv_sec)*1E6;
            elapsed+=(t6.tv_usec-t0.tv_usec);
            fprintf(stderr,"Coord: Total Elapsed Microseconds: %ld\n",elapsed);
          }
