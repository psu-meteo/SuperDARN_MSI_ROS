...
double d=ANTENNA_SEPARATION; // USED in imaging configuration
unsigned int     transmitters[16][2] = {
                 /* pci index:0-?, chip:1-4 */
                 /* Only used Imaging configuration */
                 /* TODO: read this from a site config file */
                 /* Settings are for Kodiak Radar 2011-03-18 */
                        {2,1},
                        {2,2},
                        {2,3},
                        {2,4},
		  ...
		  if (rval<0) {
		    ...
		    switch( datacode ){
		      case DDS_REGISTER_SEQ:
		        if (verbose > -1) fprintf(stdout,"\nRegister new timing sequence for dds card\n");	
...
//JDS TODO chop off first STATE_DELAY microseconds
			...
			if(verbose > 1 ) {
                          fprintf(stdout,"Setup DDS Card for PRE-trigger\n");	
                        }
                        if(numclients>0) {
                          max_seq_count=0;
                        ...
                        if (IMAGING==0) {
                          ...
                          for( i=0; i<numclients; i++) {
                            ...
                            for(ant=0;ant<MAX_TRANSMITTERS;ant++) {
                              ...
                              pci_ind=transmitters[ant][0];
                              chip=transmitters[ant][1];
                              load_frequency(ics660[pci_ind], chip, c, freq_in);
                              load_phase(ics660[pci_ind],chip,c,phase);
                              load_filter_taps(ics660[pci_ind],chip,c,T_rise,state_time);
                            }
                          }
                          one_shot_b(ics660[pci_master]);
                          gettimeofday(&t3,NULL);
