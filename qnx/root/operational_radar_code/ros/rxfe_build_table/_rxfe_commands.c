...
int build_settings_from_code(int code,struct RXFESettings *settings){
  if ((code & 1) == 1 ) settings->att1=0;
  else settings->att1=1; 
  if ((code & 2) == 2 ) settings->att2=0;
  else settings->att2=1 ;
  if ((code & 4) == 4 ) settings->att3=0;
  else settings->att3=1 ;
  if ((code & 8) == 8 ) settings->att4=0;
  else settings->att4=1 ;

  if ((code & 16) == 16 ) settings->amp1=1;
  else settings->amp1=0 ;
  if ((code & 64) == 64 ) settings->amp2=1;
  else settings->amp2=0 ;
  if ((code & 128) == 128 ) settings->amp3=1;
  else settings->amp3=0 ;

  if ((code & 32) == 32 ) settings->ifmode=1;
  else settings->ifmode=0 ;
  return 0;
...
int build_RXFE_EEPROM_code(struct RXFESettings settings){
  int code=15;
  if (settings.att1==1) code-=1;
  if (settings.att2==1) code-=2;
  if (settings.att3==1) code-=4;
