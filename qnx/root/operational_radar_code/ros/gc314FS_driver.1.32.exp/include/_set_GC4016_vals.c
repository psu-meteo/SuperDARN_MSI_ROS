...
#define GAIN_ADJ 10
//#define GAIN_ADJ 1 // JDS this setting seems to force the fallback math of optimal calculated gain 
//#define GAIN_ADJ 17 // JDS this setting seems to force the fallback math of optimal calculated gain 
...
coarse+=3;  //ADDED BY TODD TO INCREASE THE GAIN
	...
	*/
	...
	fine_gain=(int) (2*1024/gain); 				// the factor of 2 compensates for adding 1 to finalshift, giving a gain of 1
        ...
        //fprintf(stderr,"Gain_tmp: %f Gain_adj: %d\n",gain_tmp,GAIN_ADJ);
        //fflush(stderr);
        if (GAIN_ADJ<gain_tmp){
                fine_gain=(int)(2.0*(float)GAIN_ADJ*1024.0/gain);
                finalshift_adj=0;
        	//fprintf(stderr,"final shift adj: %d fine_gain: %d\n",finalshift_adj, fine_gain);
       		// fflush(stderr);
        }
        else{
	...
	if (fine_gain<=1) fine_gain=1;					// if negative or zero gain is needed, then signals will be wrong, so set fine gain to 1
       	...
       	//fprintf(stderr,"final shift: %d fine_gain: %d\n",finalshift, fine_gain);
       	//fflush(stderr);
	cicgain=pow((float) decimation,5)*pow(2,shift+scale+6*big_scale-62);

        freq_frac=(double)chanX->freq/(double)FCLOCK;
	FREQ=4294967296.0*freq_frac;	
