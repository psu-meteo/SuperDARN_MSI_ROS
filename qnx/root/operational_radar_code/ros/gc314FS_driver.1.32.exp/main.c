...
struct		 GC4016_channel_regs GC4016[4];
unsigned long    GC4016_addrs[4];
unsigned int	 msg_ptr_temp;
int		channel_on_off[4]={0,0,0,0};
int		channel_flag[4]={0,0,0,0};
int		channel_collecting[4]={0,0,0,0};
...
int get_available_DMA_channel(){
	...
	int	temp;
        int 	wait_count=0,max_wait=20000;	
	sleep.tv_sec=0;
	sleep.tv_nsec=1000;
	...
	while(1){
		...
		if(DMA_in_use[0]==0){
			...
			return 0;
		} else if(DMA_in_use[1]==0){
			DMA_in_use[1]=1;
			//fprintf(stderr, "DMA 1 in use\n");
			return 1;
		} else {
			fprintf(stderr, "DMA 0 and 1 both in use, waiting\n");
...
void * WaitForChannelA(){
	...
	remaining_samples=chanA.samples%fifo_lvl[0];
        ...
        //fprintf(stderr,"%d: Channel A: Loops: %d fifo_lvl: %d remaining: %d\n",pci_index,loops,fifo_lvl[0],remaining_samples);
                status1=*((uint32*)(BASE1+GC314FS_R3ACSR));
                if( (status1 & 0x00080000) == 0x00080000){
                        fprintf(stderr,"ERROR: wait_on_fifo Pre FIFO OVERFLOW! %x chan: %d count: %d\n",status1,0,0);
        		fflush(stderr);
                } 
                if( (status1 & 0x00040000) == 0x00040000){
                        fprintf(stderr,"wait_on_fifROR:  FIFO FULL! %x chan: %d count: %d\n",status1,0,0);
        		fflush(stderr);
                } 
                if( (status1 & 0x00020000) == 0x00020000){
                        fprintf(stderr,"ERROR: _fifo Pre FIFO LVL Reached! %x chan: %d count: %d\n",status1,0,0);
        		fflush(stderr);
                } 
	...
	for (i=0;i<loops;i++){
		channel_stat[0] = wait_on_fifo_lvl(BASE1, CHANNEL_A);
                if (channel_stat[0]!=0) {
                  fprintf(stderr,"%d Channel A: fifo_lvl ERROR:  Loop: %d lvl: %d\n",pci_index,i,fifo_lvl[0]);
                  fflush(stderr);
                  break;
                }
		...
		DMA_to_use=get_available_DMA_channel();
                if(DMA_to_use==-1) {
                  channel_stat[0]=-11; 
                  fprintf(stderr,"%d Channel A: DMA channel wait ERROR:  Loop: %d\n",pci_index,i);
                  fflush(stderr);
                  break;
                }
	  ...
	  if (remaining_samples > 0){
	    channel_stat[0]=wait_on_fifo_finish(BASE1, CHANNEL_A);
	      ...
	      DMA_to_use=get_available_DMA_channel();
              if(DMA_to_use==-1) {
                channel_stat[0]=-11; 
                fprintf(stderr,"%d Channel A: DMA channel wait ERROR:  final transfer\n",pci_index);
                fflush(stderr);
	      } else {
	        channel_stat[0]=_DMA_transfer_sg( BASE0, BASE1, 0x40000, &physical_addr[0], 4*scount, remaining_samples, 0, DMA_to_use);
	        DMA_in_use[DMA_to_use]=0;
	        scount += remaining_samples;
              ...
              fprintf(stderr,"%d Channel A: fifo_finish ERROR lvl: %d\n",pci_index,fifo_lvl[0]);
              fflush(stderr);
            }
	  }
        } 
        //fprintf(stderr,"%d Channel A: Stopping Aquisition\n",pci_index);
        //fflush(stderr);
        *((uint32*)(BASE1+GC314FS_R2ACSR)) |= 0x00000004;
        *((uint32*)(BASE1+GC314FS_R1ACSR)) |= 0x00000004;
        *((uint32*)(BASE1+GC314FS_R3ACSR)) |= 0x00000004;
	temp=clock_gettime(CLOCK_REALTIME, &stop);
        channel_collecting[0]=0;
...
void * WaitForChannelB(){
	...
	remaining_samples=chanB.samples%fifo_lvl[1];
        ...
        //fflush(stderr);
                status1=*((uint32*)(BASE1+GC314FS_R3BCSR));
                if( (status1 & 0x00080000) == 0x00080000){
                        fprintf(stderr,"ERROR: wait_on_fifo Pre FIFO OVERFLOW! %x chan: %d count: %d\n",status1,1,0);
        		fflush(stderr);
                } 
	...
	for (i=0;i<loops;i++){
                ...
                if (channel_stat[1]!=0) {
                  fprintf(stderr,"%d Channel B: fifo_lvl ERROR:  Loop: %d lvl: %d\n",pci_index,i,fifo_lvl[1]);
                  fflush(stderr);
                  break;
                }
                temp=clock_gettime(CLOCK_REALTIME, &stop);
		DMA_to_use=get_available_DMA_channel();
                if(DMA_to_use==-1) {
                  channel_stat[1]=-11; 
                  fprintf(stderr,"%d Channel B: DMA channel wait ERROR:  Loop: %d\n",pci_index,i);
                  fflush(stderr);
                  break;
                }
	  ...
	  if (remaining_samples > 0){
	    channel_stat[1]=wait_on_fifo_finish(BASE1, CHANNEL_B);
	      ...
	      DMA_to_use=get_available_DMA_channel();
              if(DMA_to_use==-1) {
                channel_stat[1]=-11; 
                fprintf(stderr,"%d Channel B: DMA channel wait ERROR:  final transfer\n",pci_index);
                fflush(stderr);
	      } else {
	        channel_stat[1]=_DMA_transfer_sg(BASE0, BASE1, 0x44000, &physical_addr[3], 4*scount, remaining_samples, 0, DMA_to_use);
	        DMA_in_use[DMA_to_use]=0;
	        scount += remaining_samples;
              }
              if( channel_stat[1]!=0) {
...
void * WaitForChannelC(){
	...
	for (i=0;i<loops;i++){
                ...
                if (channel_stat[2]!=0) {
                  fprintf(stderr,"%d Channel C: fifo_lvl ERROR:  Loop: %d lvl: %d\n",pci_index,i,fifo_lvl[2]);
                  fflush(stderr);
                  break;
                }
                temp=clock_gettime(CLOCK_REALTIME, &stop);
		DMA_to_use=get_available_DMA_channel();
                if(DMA_to_use==-1) {
                  channel_stat[2]=-11; 
                  fprintf(stderr,"%d Channel C: DMA channel wait ERROR:  Loop: %d\n",pci_index,i);
                  fflush(stderr);
                  break;
                }
	  ...
	  if (remaining_samples > 0){
	    channel_stat[2]=wait_on_fifo_finish(BASE1, CHANNEL_C);
	      ...
	      DMA_to_use=get_available_DMA_channel();
              if(DMA_to_use==-1) {
                channel_stat[2]=-11; 
                fprintf(stderr,"%d Channel C: DMA channel wait ERROR:  final transfer\n",pci_index);
                fflush(stderr);
	      } else {
	        channel_stat[2]=_DMA_transfer_sg(BASE0, BASE1, 0x48000, &physical_addr[6], 4*scount, remaining_samples, 0, DMA_to_use);
	        DMA_in_use[DMA_to_use]=0;
	        scount += remaining_samples;
	      }
              if( channel_stat[2]!=0) {
...
void * WaitForChannelD(){
	...
	remaining_samples=chanD.samples%fifo_lvl[3];
        //fprintf(stderr,"%d: Channel D: Start Wait %d\n",pci_index,channel_flag[3]);
        //fprintf(stderr,"%d: Channel D: Loops: %d fifo_lvl: %d remaining: %d\n",pci_index,loops,fifo_lvl[3],remaining_samples);
                status1=*((uint32*)(BASE1+GC314FS_R3DCSR));
                //fprintf(stderr,"wait_on_fifo Pre R3DCSR: %x chan: %d count: %d\n",status1,3,0);
        	fflush(stderr);
                if( (status1 & 0x00080000) == 0x00080000){
                        fprintf(stderr,"ERROR: wait_on_fifo Pre FIFO OVERFLOW! %x chan: %d count: %d\n",status1,3,0);
        		fflush(stderr);
                ...
                if( (status1 & 0x00040000) == 0x00040000){
                        fprintf(stderr,"ERRORi: wait_on_fifo Pre FIFO FULL! %x chan: %d count: %d\n",status1,3,0);
        		fflush(stderr);
                } 
                if( (status1 & 0x00020000) == 0x00020000){
                        fprintf(stderr,"ERROR: _fifo Pre FIFO LVL Reached! %x chan: %d count: %d\n",status1,3,0);
	...
	for (i=0;i<loops;i++){
                ...
                if (channel_stat[3]!=0) {
                  fprintf(stderr,"%d Channel D: fifo_lvl ERROR:  Loop: %d lvl: %d\n",pci_index,i,fifo_lvl[3]);
                  fflush(stderr);
                  break;
                }
                temp=clock_gettime(CLOCK_REALTIME, &stop);
		DMA_to_use=get_available_DMA_channel();
                if(DMA_to_use==-1) {
                  channel_stat[3]=-11; 
                  fprintf(stderr,"%d Channel D: DMA channel wait ERROR:  Loop: %d\n",pci_index,i);
                  fflush(stderr);
                  break;
                }
	  ...
	  if (remaining_samples > 0){
	    channel_stat[3]=wait_on_fifo_finish(BASE1, CHANNEL_D);
	      ...
	      DMA_to_use=get_available_DMA_channel();
              if(DMA_to_use==-1) {
                channel_stat[3]=-11; 
                fprintf(stderr,"%d Channel D: DMA channel wait ERROR:  final transfer\n",pci_index);
                fflush(stderr);
	      } else {
	        channel_stat[3]=_DMA_transfer_sg(BASE0, BASE1, 0x4c000, &physical_addr[9], 4*scount, remaining_samples, 0, DMA_to_use);
	        DMA_in_use[DMA_to_use]=0;
	        scount += remaining_samples;
              }
              if( channel_stat[3]!=0) {
...
int main( int argc, char **argv){
 ...
 //MAKE SURE A DEVICE NUMBER WAS SPECIFIED AT THE COMMAND LINE 
	...
	//temp=_config_PLLs(BASE1, CLOCK_EXTERNAL, FCLOCK);
        for(i=0;i<3;i++) {
	 fprintf(stderr, "PLL configuration Attempt %d\n",i);
	  temp=_config_PLLs(BASE1, CLOCK_EXTERNAL, FCLOCK);
	// *******************************************************************************************
	  if(temp==-1) {
	    fprintf(stderr, "PLL configuration failed \n");
	  } else {
		 fprintf(fout, "PLL configuration sucessfull!\n");
		 break;
          }
	  usleep(200);
	}
	...
	for (i=0;i<12;i++){
		...
		temp=_create_DMA_buff(&virtual_addr[i], &physical_addr[i], 1048576);
                fprintf(stderr,"DMA buff: %d virt: %x phys: %x\n",i,virtual_addr[i],physical_addr[i]);
		if (temp==-1){
			fprintf(fout, "ERROR MAKING DMA BUFFERS!\n");
			break;
		}
//                memset((void *)virtual_addr[i],0,samples_to_collect*4+header_size);
	}
	...
	temp=_set_GC4016_vals(&chanA, &GC4016[0], resampcoeffs, resampratios);
	temp=_set_GC4016_vals(&chanB, &GC4016[1], resampcoeffs, resampratios);
	temp=_set_GC4016_vals(&chanC, &GC4016[2], resampcoeffs, resampratios);
	temp=_set_GC4016_vals(&chanD, &GC4016[3], resampcoeffs, resampratios);
	GC4016_addrs[0]=(unsigned int)&GC4016[0];
	GC4016_addrs[1]=(unsigned int)&GC4016[1];
	GC4016_addrs[2]=(unsigned int)&GC4016[2];
	GC4016_addrs[3]=(unsigned int)&GC4016[3];
	c=(short *)&GC4016;
...
int io_devctl (resmgr_context_t *ctp, io_devctl_t *msg, RESMGR_OCB_T *ocb){
	...
	switch(msg->i.dcmd){
			...
			switch(rx_data->channel){
				case CHANNEL_A:
					if(PRINT) printf("channel A\n");
                                        if(channel_on_off[0]) printf("channel A already on!\n");
					channel_on_off[0]=1;
					break;
				case CHANNEL_B:
					if(PRINT) printf("channel B\n");
                                        if(channel_on_off[1]) printf("channel B already on!\n");
					channel_on_off[1]=1;
					break;	
				case CHANNEL_C:
					if(PRINT) printf("channel C\n");
                                        if(channel_on_off[2]) printf("channel C already on!\n");
					channel_on_off[2]=1;
					break;
				case CHANNEL_D:
					if(PRINT) printf("channel D\n");
                                        if(channel_on_off[3]) fprintf(stderr,"channel D already on!\n");
					channel_on_off[3]=1;
					break;
			}
			return(_RESMGR_PTR(ctp, &msg->o, sizeof(msg->o)+nbytes));
		...
		case GC314_LOAD_GC4016S:
			...
			temp=_config_GC4016(BASE1, 1, GC4016_addrs, resampcoeffs, resampratios);
			return(_RESMGR_PTR(ctp, &msg->o, sizeof(msg->o)+nbytes));
			break;
		case GC314_SET_RDA:
			...
			switch(rx_data->RDA_data.channel){
				case CHANNEL_A:
			          if(channel_waiting[0]) {
                                    fprintf(stderr,"RDA: Channel A already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[0]) {
                                    fprintf(stderr,"RDA: Channel A already collecting\n");
                                    fflush(stderr);
			          }

//                                  fprintf(stderr,"RDA: Initial R3ACSR: %x\n",*((uint32*)(BASE1+GC314FS_R3ACSR)));
//                                  fprintf(stderr,"RDA: Initial RxA frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1AID)),
//                                        *((uint16*)(BASE1+GC314FS_R2AID)),
//                                        *((uint16*)(BASE1+GC314FS_R3AID)));

                                  temp=_clear_FIFO(BASE1, 1, CHANNEL_A);

//                                  fprintf(stderr,"RDA: Mid R3ACSR: %x\n",
//				  	*((uint32*)( BASE1+GC314FS_R3ACSR)));

                                  temp=_set_RDA(BASE1, 1, chanA.samples, skip[0], fifo_lvl[0], CHANNEL_A);
//
//                                  fprintf(stderr,"RDA: Final R3ACSR: %x\n",*((uint32*)(BASE1+GC314FS_R3ACSR)));
//                                  fprintf(stderr,"RDA: Final  frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1AID)),
//                                        *((uint16*)(BASE1+GC314FS_R2AID)),
//                                        *((uint16*)(BASE1+GC314FS_R3AID)));
				...
				case CHANNEL_B:
			          if(channel_waiting[1]) {
                                    fprintf(stderr,"RDA: Channel B already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[1]) {
                                    fprintf(stderr,"RDA: Channel B already collecting\n");
                                    fflush(stderr);
			          }

//                                  fprintf(stderr,"RDA: Initial R3BCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3BCSR)));
//                                  fprintf(stderr,"RDA: Initial RxB frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1BID)),
//                                        *((uint16*)(BASE1+GC314FS_R2BID)),
//                                        *((uint16*)(BASE1+GC314FS_R3BID)));

                                        temp=_clear_FIFO(BASE1, 1, CHANNEL_B);

//                                  fprintf(stderr,"RDA: Mid R3BCSR: %x\n",
//				  	*((uint32*)( BASE1+GC314FS_R3BCSR)));

					temp=_set_RDA(BASE1, 1, chanB.samples, skip[1], fifo_lvl[1], CHANNEL_B);

//                                  fprintf(stderr,"RDA: Final R3BCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3BCSR)));
//                                  fprintf(stderr,"RDA: Final  frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1BID)),
//                                        *((uint16*)(BASE1+GC314FS_R2BID)),
//                                        *((uint16*)(BASE1+GC314FS_R3BID)));
					...
					break;
				case CHANNEL_C:
			          if(channel_waiting[2]) {
                                    fprintf(stderr,"RDA: Channel C already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[2]) {
                                    fprintf(stderr,"RDA: Channel C already collecting\n");
                                    fflush(stderr);
			          }
//                                  fprintf(stderr,"RDA: Initial R3CCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3CCSR)));
//                                  fprintf(stderr,"RDA: Initial RxC frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1CID)),
//                                        *((uint16*)(BASE1+GC314FS_R2CID)),
//                                        *((uint16*)(BASE1+GC314FS_R3CID)));

                                        temp=_clear_FIFO(BASE1, 1, CHANNEL_C);

//                                  fprintf(stderr,"RDA: Mid R3CCSR: %x\n",
//				  	*((uint32*)( BASE1+GC314FS_R3CCSR)));

					temp=_set_RDA(BASE1, 1, chanC.samples, skip[2], fifo_lvl[2], CHANNEL_C);

//                                  fprintf(stderr,"RDA: Final R3CCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3CCSR)));
//                                  fprintf(stderr,"RDA: Final  frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1CID)),
//                                        *((uint16*)(BASE1+GC314FS_R2CID)),
//                                        *((uint16*)(BASE1+GC314FS_R3CID)));
					break;
				case CHANNEL_D:
			          if(channel_waiting[3]) {
                                    fprintf(stderr,"RDA: Channel D already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[3]) {
                                    fprintf(stderr,"RDA: Channel D already collecting\n");
                                    fflush(stderr);
			          }
//                                  fprintf(stderr,"RDA: Initial R3DCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3DCSR)));
//                                  fprintf(stderr,"RDA: Initial RxD frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1DID)),
//                                        *((uint16*)(BASE1+GC314FS_R2DID)),
//                                        *((uint16*)(BASE1+GC314FS_R3DID)));
                                        temp=_clear_FIFO(BASE1, 1, CHANNEL_D);
//                                  fprintf(stderr,"RDA: Mid R3DCSR: %x\n",
//				  	*((uint32*)( BASE1+GC314FS_R3DCSR)));

					temp=_set_RDA(BASE1, 1, chanD.samples, skip[3], fifo_lvl[3], CHANNEL_D);

//                                  fprintf(stderr,"RDA: Final R3DCSR: %x\n",*((uint32*)(BASE1+GC314FS_R3DCSR)));
//                                  fprintf(stderr,"RDA: Final  frame_count: %d %d %d\n",
//                                        *((uint16*)(BASE1+GC314FS_R1DID)),
//                                        *((uint16*)(BASE1+GC314FS_R2DID)),
//                                        *((uint16*)(BASE1+GC314FS_R3DID)));
			...
			switch(rx_data->samples_data.channel){
				case CHANNEL_A:
                                       chanA.samples=rx_data->samples_data.samples;
                                        //fprintf(stderr,"channel A req samples=%d\n", chanA.samples);

                                        if(chanA.samples <= FIFO_LVL) fifo_lvl[0]=chanA.samples;
                                        else {
                                          fifo_lvl[0]=FIFO_LVL;
                                          chanA.samples=ceil((double)rx_data->samples_data.samples / (double)FIFO_LVL)*FIFO_LVL;
                                        }
                                        //fprintf(stderr,"channel A samples=%d\n", chanA.samples);
                                        //fflush(stderr);
                                        break;
...
//					break;
				case CHANNEL_B:
                                       chanB.samples=rx_data->samples_data.samples;
                                        //fprintf(stderr,"channel B req samples=%d\n", chanB.samples);

                                        if(chanB.samples <= FIFO_LVL) fifo_lvl[1]=chanB.samples;
                                        else {
                                          fifo_lvl[1]=FIFO_LVL;
                                          chanB.samples=ceil((double)rx_data->samples_data.samples / (double)FIFO_LVL)*FIFO_LVL;
                                        }
                                        //fprintf(stderr,"channel B samples=%d\n", chanB.samples);
                                        //fflush(stderr);
                                        break;
//					chanB.samples=rx_data->samples_data.samples;
//					if(chanB.samples < FIFO_LVL) fifo_lvl[1]=chanB.samples;
...
//					break;	
				case CHANNEL_C:
                                       chanC.samples=rx_data->samples_data.samples;
                                        //fprintf(stderr,"channel C req samples=%d\n", chanC.samples);

                                        if(chanC.samples <= FIFO_LVL) fifo_lvl[2]=chanC.samples;
                                        else {
                                          fifo_lvl[2]=FIFO_LVL;
                                          chanC.samples=ceil((double)rx_data->samples_data.samples / (double)FIFO_LVL)*FIFO_LVL;
                                        }
                                        //fprintf(stderr,"channel C samples=%d\n", chanC.samples);
                                        //fflush(stderr);
                                        break;
//					chanC.samples=rx_data->samples_data.samples;
//					if(chanC.samples < FIFO_LVL) fifo_lvl[2]=chanC.samples;
...
//					break;
				case CHANNEL_D:
                                       chanD.samples=rx_data->samples_data.samples;
                                        //fprintf(stderr,"channel D req samples=%d\n", chanD.samples);

                                        if(chanD.samples <= FIFO_LVL) fifo_lvl[3]=chanD.samples;
                                        else {
                                          fifo_lvl[3]=FIFO_LVL;
                                          chanD.samples=ceil((double)rx_data->samples_data.samples / (double)FIFO_LVL)*FIFO_LVL;
                                        }
                                        //fprintf(stderr,"channel D samples=%d\n", chanD.samples);
                                        //fflush(stderr);
                                        break;
			}
			return(_RESMGR_PTR(ctp, &msg->o, sizeof(msg->o)+nbytes));
		...
		case GC314_UPDATE_CHANNEL:
			...
			switch(rx_data->channel){
				case CHANNEL_A:
					if(PRINT) printf("channel A\n");
			          if(channel_waiting[0]) {
                                    fprintf(stderr,"UPDATE: Channel A already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[0]) {
                                    fprintf(stderr,"UPDATE: Channel A already collecting\n");
                                    fflush(stderr);
			          }
					skip[0]=_set_GC4016_vals(&chanA, &GC4016[0], resampcoeffs, resampratios);
					break;
				case CHANNEL_B:
					if(PRINT) printf("channel B\n");
			          if(channel_waiting[1]) {
                                    fprintf(stderr,"UPDATE: Channel B already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[1]) {
                                    fprintf(stderr,"UPDATE: Channel B already collecting\n");
                                    fflush(stderr);
			          }
					skip[1]=_set_GC4016_vals(&chanB, &GC4016[1], resampcoeffs, resampratios);
					break;	
				case CHANNEL_C:
					if(PRINT) printf("channel C\n");
			          if(channel_waiting[2]) {
                                    fprintf(stderr,"UPDATE: Channel C already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[2]) {
                                    fprintf(stderr,"UPDATE: Channel C already collecting\n");
                                    fflush(stderr);
			          }
					skip[2]=_set_GC4016_vals(&chanC, &GC4016[2], resampcoeffs, resampratios);
					break;
				case CHANNEL_D:
					if(PRINT) printf("channel D\n");
			          if(channel_waiting[3]) {
                                    fprintf(stderr,"UPDATE: Channel D already waiting\n");
                                    fflush(stderr);
			          }
			          if(channel_collecting[3]) {
                                    fprintf(stderr,"UPDATE: Channel D already collecting\n");
                                    fflush(stderr);
			          }
					skip[3]=_set_GC4016_vals(&chanD, &GC4016[3], resampcoeffs, resampratios);
					break;
			}
			return(_RESMGR_PTR(ctp, &msg->o, sizeof(msg->o)+nbytes));
			break;
		case GC314_START_COLLECTION:
                        ...
                        //}
//                        for(i=0;i<12;i++) {
//                          memset((void *)virtual_addr[i],0,500*4+header_size);
//                        }

                        if(helperA!=NULL) {
                            fprintf(stderr,"Collection: helperA already active\n");
                            fflush(stderr);
                            pthread_join(helperA, NULL);
                            helperA=NULL;
                        }
                        if(helperB!=NULL) {
                            fprintf(stderr,"Collection: helperB already active\n");
                            fflush(stderr);
                            pthread_join(helperB, NULL);
                            helperB=NULL;
                        }
                        if(helperC!=NULL) {
                            fprintf(stderr,"Collection: helperC already active\n");
                            fflush(stderr);
                            pthread_join(helperC, NULL);
                            helperC=NULL;
                        }
                        if(helperD!=NULL) {
                            fprintf(stderr,"Collection: helperD already active\n");
                            fflush(stderr);
                            pthread_join(helperD, NULL);
                            helperD=NULL;
                        }
			...
			if(channel_on_off[0]) {
                          channel_on_off[0]=0;
			  if(channel_collecting[0]) {
                            fprintf(stderr,"Collection: Channel A on and already collecting\n");
                            fflush(stderr);
			  }
                          channel_collecting[0]=1;
                          if(helperA!=NULL) {
                            pthread_join(helperA, NULL);
                            helperA=NULL;
                          }
			...
			if(channel_on_off[1]) {
                          channel_on_off[1]=0;
			  if(channel_collecting[1]) {
                            fprintf(stderr,"Collection: Channel B on and already collecting\n");
                            fflush(stderr);
			  }
                          channel_collecting[1]=1;
                          if(helperB!=NULL) {
                            pthread_join(helperB, NULL);
                            helperB=NULL;
                          }
                          pthread_create(&helperB,NULL,WaitForChannelB,NULL);
                        }
			if(channel_on_off[2]) {
                          channel_on_off[2]=0;
			  if(channel_collecting[2]) {
                            fprintf(stderr,"Collection: Channel C on and already collecting\n");
                            fflush(stderr);
			  }
                          channel_collecting[2]=1;
                          if(helperC!=NULL) {
                            pthread_join(helperC, NULL);
                            helperC=NULL;
                          }
                          pthread_create(&helperC,NULL,WaitForChannelC,NULL);
                        }
			if(channel_on_off[3]) {
                          channel_on_off[3]=0;
			  if(channel_collecting[3]) {
                            fprintf(stderr,"Collection: Channel D on and already collecting\n");
                            fflush(stderr);
			  }
                          channel_collecting[3]=1;
                          if(helperD!=NULL) {
                            pthread_join(helperD, NULL);
                            helperD=NULL;
                          }
                          pthread_create(&helperD,NULL,WaitForChannelD,NULL);
                        }
			for(i=0;i<4;i++) {
			  if(channel_collecting[i]==1) {
			    count=0;
                            while((channel_flag[i]==0 ) && (count < 10)) {
//			      if(i==3) {
//				fprintf(stderr,"Start Collection: D init flag: %d count: %d\n",channel_flag[3],count);
//                              }	
			      usleep(10);
			      count++;	
                            }
			    if(count >= 10 ) {
			  	fprintf(stderr,"Collection: Channel %d Helper thread took too long to Init\n",i);
			  	fflush(stderr);
                            }	
			  }
			}		  
		...
		case GC314_WAIT_FOR_DATA:
			...
			*/
                        ...
                        switch(channel) {
                          case 0:  
			    if(channel_waiting[0]) {
                              fprintf(stderr,"Waiting: Channel A already waiting\n");
                              fflush(stderr);
			    }
                            channel_waiting[0]=1;
                          ...
                          case 1: 
			    if(channel_waiting[1]) {
                              fprintf(stderr,"Waiting: Channel B already waiting\n");
                              fflush(stderr);
			    }
                            channel_waiting[1]=1;
                          ...
                          case 2:
			    if(channel_waiting[2]) {
                              fprintf(stderr,"Waiting: Channel C already waiting\n");
                              fflush(stderr);
			    }
                            channel_waiting[2]=1;
                          ...
                          case 3:
			    if(channel_waiting[3]) {
                              fprintf(stderr,"Waiting: Channel D already waiting\n");
                              fflush(stderr);
			    }
                            channel_waiting[3]=1;
		...
		case GC314_ABORT_CHANNEL:
                       printf("Aborting channel: ");
                       switch(rx_data->channel){
                               case CHANNEL_A:
                                       printf("channel A : %d %d %d\n",channel_on_off[0],channel_collecting[0],channel_waiting[0]);
                                       channel_on_off[0]=0;
                                       channel_collecting[0]=0;
                                       channel_waiting[0]=0;
                                       *((uint32*)(BASE1+GC314FS_R1ACSR)) |= 0x00000004;
                                       ...
                                       *((uint32*)(BASE1+GC314FS_R3ACSR)) |= 0x00000004;
                                       break;
                               case CHANNEL_B:
                                       printf("channel B : %d %d %d\n",channel_on_off[1],channel_collecting[1],channel_waiting[1]);
                                       channel_on_off[1]=0;
                                       channel_collecting[1]=0;
                                       channel_waiting[1]=0;
                  ...
                  //fflush(stderr);
                                       ...
                                       *((uint32*)(BASE1+GC314FS_R3BCSR)) |= 0x00000004;
                                       break;
                               case CHANNEL_C:
                                       printf("channel C : %d %d %d\n",channel_on_off[2],channel_collecting[2],channel_waiting[2]);
                                       channel_on_off[2]=0;
                                       channel_collecting[2]=0;
                                       channel_waiting[2]=0;
                                       *((uint32*)(BASE1+GC314FS_R1CCSR)) |= 0x00000004;
                                       ...
                                       *((uint32*)(BASE1+GC314FS_R3CCSR)) |= 0x00000004;
                                       break;
                               case CHANNEL_D:
                                       printf("channel D : %d %d %d\n",channel_on_off[3],channel_collecting[3],channel_waiting[3]);
                                       channel_on_off[3]=0;
                                       channel_collecting[3]=0;
                                       channel_waiting[3]=0;
                                       printf("channel D\n");
