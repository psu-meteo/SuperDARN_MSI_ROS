...
void gc314SetGlobalReset(int filedes, int state){
	
	int retval;
	int status=0;
	struct S_global_reset_data   to_send;
	
#ifdef __QNX__

	to_send.state=state;

	retval = devctl(filedes, GC314_SET_GLOBAL_RESET, &to_send, sizeof(to_send), &status);
	if(retval != EOK){
		fprintf(stderr,"Error in gc314fs device descriptor %d: %s\n", filedes, strerror(errno));
		exit(EXIT_FAILURE);
	}
	if(status != GC314_SET_GLOBAL_RESET){
