...
#define S_bit 0x01 /* sample bit */
#define A_bit 0x08 /* attenuator bit */
#define R_bit 0x02 /* receive bit */
...
struct TSGStateDefinition {
	 int rf_code;		/* code bits asserted when in this state */
...
/********|*********|*********|*********|*********|*********|*********|*********/
  ...
  while (cnt.n_smp < N_sample) {
    code = states[state].rf_code | scope;

    /* fudge to trigger the GC214TS card after the first tx */

    if ((tsg->gort == 1) && (code & X_bit)) {
      scope = noscope;
    }
