...
int main(int argc,char *argv[]) {
  ...
  if (fast) sprintf(progname,"stutterscan (fast)");
  else sprintf(progname,"stutterscan");
  ...
  do {
    ...
    while(stutter_count<scan_stutter) {
      if (SiteStartScan() !=0) break;
      ErrLog(errlog.sock,progname,"Waiting for scan stutter boundary."); 
      if ((exitpoll==0) && (scannowait==0)) SiteEndScan(scnsc,scnus);
      stutter_count++;
    }
    stutter_count=0;
