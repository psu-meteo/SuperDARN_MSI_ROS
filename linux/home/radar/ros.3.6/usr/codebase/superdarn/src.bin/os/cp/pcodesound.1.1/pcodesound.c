 ...
 */
          ...
          sprintf(logtxt, "SND FCLR: base freq: %d bandwidth: %d", sounder_freq, frqrng);
          ErrLog(errlog.sock,progname, logtxt);
          tfreq=SiteFCLR( sounder_freq, sounder_freq + frqrng);
/*
          sprintf(logtxt, "SND Assigned FRQ: %d Noise: %d", tfreq ,noise);
          ErrLog(errlog.sock,progname, logtxt);
*/
          /*SiteSetFreq(tfreq);*/
	  ...
	  /* JDS: Not needed, this slows things down terribly */
          /*tsgid= SiteTimeSeq(ptab);*/
          nave= SiteIntegrate( lags);
          if (nave < 0) {
