...
int main(int argc,char *argv[]) {
  char progid[80]={"spaletarotate"};
  char progname[256]="spaletarotate";
  char modestr[32];
...
/* Standard pulse sequence */
  int standard_ptab[8] = {0,14,22,24,27,31,42,43};
  int standard_lags[LAG_SIZE][2] = {
    { 0, 0},            /*  0 */
    {42,43},            /*  1 */
    {22,24},            /*  2 */
    {24,27},            /*  3 */
...
/* Extended pulse sequence */
  int extended_ptab[16] = {0,4,19,42,78,127,191,270,364,474,600,745,905,1083,1280,1495};
  int extended_lags[LAG_SIZE][2] = {
  {1495,1495},          /*  0 */
  {0,4},                /*  1 */
  {4,19},               /*  2 */
  {0,19},               /*  3 */
...
/* Integration period variables */
  ...
  int beams=0;
  int skip;
  /* Variables associated with freq rotate */
  int rotaterep=1;
  int *rotatefreqs=NULL;
  struct timeval *rotateskip=NULL;
  ...
  struct arg_lit  *al_extended    = arg_lit0(NULL, "extended","Use extended pulse sequence"); 
  /* Now lets define the integer valued arguments */
  struct arg_int  *ai_rotatesc   = arg_int0(NULL, "rsec", NULL,"Integer Seconds for freq rotation");
  struct arg_int  *ai_rotateus   = arg_int0(NULL, "rusec", NULL,"Integer Usecs for freq rotation");
  struct arg_int  *ai_rotaterep  = arg_int0(NULL, "rrep", NULL,"Repetitions to use for freq rotation");
  struct arg_int  *ai_rotatefreq  = arg_intn(NULL, "rfreq", NULL,0,10,"Add a freq in kHz to use for freq rotation");
  ...
  void* argtable[] = {al_help,al_debug,al_test,al_discretion, al_fast, al_nowait, al_onesec, \
                      ai_rotatesc,ai_rotateus,ai_rotaterep,ai_rotatefreq, \
                      ai_baud, ai_tau, ai_nrang, ai_frang, ai_rsep, ai_dt, ai_nt, ai_df, ai_nf, ai_fixfrq, ai_xcf, ai_ep, ai_sp, ai_bp, ai_sb, ai_eb, ai_cnum, \
                      as_ros, as_ststr, as_libstr,as_verstr,ai_clrskip,al_extended,al_clrscan,ai_cpid,ae_argend};
...
/* load any provided argument values overriding default values provided by SiteStart */ 
  ...
  if (ai_rotaterep->count)rotaterep=ai_rotaterep->ival[0];
  if (ai_rotatefreq->count){
    rotatefreqs=(int*)calloc(ai_rotatefreq->count+1,sizeof(int));
    rotateskip=(struct timeval*)calloc(ai_rotatefreq->count+1,sizeof(struct timeval));
    gettimeofday(&rotateskip[0],NULL);
    for(i=0;i<ai_rotatefreq->count;i++){
      rotatefreqs[i+1]=ai_rotatefreq->ival[i];
      gettimeofday(&rotateskip[i+1],NULL);
    }
  } else {
    rotatefreqs=(int*)calloc(1,sizeof(int));
    rotateskip=(int*)calloc(1,sizeof(int));
    gettimeofday(&rotateskip[0],NULL);
    ai_rotatefreq->count=0;
  }
...
/* TODO: JDS: You can not make any day night changes that impact TR gate timing at dual site locations. Care must be taken with day night operation*/      
      ...
      if(ai_fixfrq->ival[0]>0) {
        rotatefreqs[0]=ai_fixfrq->ival[0];
        noise=0; 
      }
      for(j=0;j<rotaterep;j++){
        for(i=0;i<ai_rotatefreq->count+1;i++){
          stfrq=rotatefreqs[i];
          tfreq=stfrq;
          ErrLog(errlog.sock,progname,"Starting Integration.");
          sprintf(logtxt," Int parameters:: rsep: %d mpinc: %d sbm: %d ebm: %d nrang: %d nbaud: %d scannowait: %d clrskip_secs: %d clrscan: %d cpid: %d findex: %d",
              rsep,mpinc,sbm,ebm,nrang,nbaud,al_nowait->count,ai_clrskip->ival[0],al_clrscan->count,cp,i);
          ErrLog(errlog.sock,progname,logtxt);
          ...
          gettimeofday(&t1,NULL);
          elapsed_secs=t1.tv_sec-rotateskip[i].tv_sec;
          ...
          if((elapsed_secs >= ai_clrskip->ival[0])||(startup==1)) {
            ...
            rotateskip[i].tv_sec=t1.tv_sec;
            rotateskip[i].tv_usec=t1.tv_usec;
          }
