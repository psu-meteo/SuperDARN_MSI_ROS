...
void SiteMcmExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteMcmExit: Sig %d: %d\n",signum,MCM_exit_flag); 
      cancel_count++;
      MCM_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteMcmExit: Sig %d: %d\n",signum,MCM_exit_flag); 
...
int SiteMcmStart(char *host) {
  signal(SIGPIPE,SiteMcmExit);
  signal(SIGINT,SiteMcmExit);
  signal(SIGUSR1,SiteMcmExit);
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteMcmStart: rxchn=%d\n",rxchn);
...
int SiteMcmSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Mcm_%d_%d",rnum,cnum);
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.mcm%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.mcm%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
int SiteMcmStartIntt(int sec,int usec) {
  ...
  SiteMcmExit(0);
  if (debug) {
    fprintf(stderr,"SiteMcmStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteMcmStartInt: end\n");
  }
  return 0;
...
int SiteMcmIntegrate(int (*lags)[2]) {
  ...
  uint32 uQ32,uI32;
  if (debug) {
    fprintf(stderr,"MCM SiteIntegrate: start\n");
  }
  SiteMcmExit(0);
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"MCM GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"MCM GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"MCM GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"MCM GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"MCM Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"MCM nave=%d\n",nave);
      fprintf(stderr,"MCM dprm.status=%d\n",dprm.status);
    }
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.mcm%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"MCM seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"MCM seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"MCM seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"MCM seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"MCM seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"MCM seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"MCM seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"MCM seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"MCM seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"MCM seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"MCM seq %d :: ACFCalculate xcf\n",nave);
          ACFCalculate(&tsgprm,(int16 *) dest,rngoff,skpnum!=0,
        ...
        fprintf(stderr,"MCM seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"MCM seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"MCM SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"MCM SiteIntegrate: end: nave: %d\n",nave);
   }
