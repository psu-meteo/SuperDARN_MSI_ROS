...
void SiteTstExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteTstExit: Sig %d: %d\n",signum,TST_exit_flag); 
      cancel_count++;
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.tst%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.tst%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
*/
    ...
    if (debug) {
      ...
      fprintf(stderr,"TST dprm.status=%d\n",dprm.status);
    }
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.tst%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        ...
        fprintf(stderr,"TST seq %d :: rdata.main 16bit :\n",nave);
...
*/
   ...
   if (debug) {
     ...
     fprintf(stderr,"TST SiteIntegrate: end: nave: %d\n",nave);
   }
