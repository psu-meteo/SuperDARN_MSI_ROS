...
void SiteKodExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteKodExit: Sig %d: %d\n",signum,KOD_exit_flag); 
      cancel_count++;
      KOD_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteKodExit: Sig %d: %d\n",signum,KOD_exit_flag); 
...
int SiteKodStart(char *host) {
  signal(SIGPIPE,SiteKodExit);
  signal(SIGINT,SiteKodExit);
  signal(SIGUSR1,SiteKodExit);
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteKodStart: rxchn=%d\n",rxchn);
...
int SiteKodSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Kod_%d_%d",rnum,cnum);
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.kod%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.kod%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
int SiteKodStartIntt(int sec,int usec) {
  ...
  SiteKodExit(0);
  if (debug) {
    fprintf(stderr,"SiteKodStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteKodStartInt: end\n");
  }
  return 0;
...
int SiteKodIntegrate(int (*lags)[2]) {
  ...
  struct tm tstruct;
  time_t ttime;
  long nsecs, last_nsecs;
  int use_gps_event_time;
  char filename[256];
  struct ROSMsg smsg,rmsg;
  ...
  nsecs=time_now.tv_sec*1E9+time_now.tv_nsec; 
  last_nsecs=nsecs-1E9;
  gmtime_r(&ttime,&tstruct);
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"KOD GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"KOD GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"KOD GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"KOD GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"KOD Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"KOD nave=%d\n",nave);
      fprintf(stderr,"KOD dprm.status=%d\n",dprm.status);
    }
    ttime=dprm.event_secs;
    nsecs=dprm.event_secs*1E9+dprm.event_nsecs;
    use_gps_event_time=1;
/*
    fprintf(stderr,"GPS Event time: %ld last event: %ld Soft: %ld\n",
      (long)nsecs,(long)last_nsecs,(long)time_now.tv_sec*1E9+time_now.tv_nsec);
*/
    if(dprm.event_capture) {
      if ( ttime < 100 ) {
        fprintf(stderr,"GPS Event time in 1970, GPS clock not initialized?\n");
        ttime=time_now.tv_sec;
        nsecs=time_now.tv_sec*1E9+time_now.tv_nsec;
        use_gps_event_time=0;
      } else {
        if ( nsecs-last_nsecs < 0 ) {
          fprintf(stderr,"GPS Event time in the past compared to last event\n");
          fprintf(stderr,"GPS Event %d time: %ld last event: %ld Soft: %ld\n",
            (int)dprm.event_capture, (long)nsecs,(long)last_nsecs,(long)(time_now.tv_sec*1E9+time_now.tv_nsec));
          ttime=time_now.tv_sec;
          nsecs=time_now.tv_sec*1E9+time_now.tv_nsec;
          use_gps_event_time=0;
        } else {
          if ( nsecs-last_nsecs == 0 ) {
            fprintf(stderr,"GPS Event time is the same as last event %ld %ld\n",
              (long)nsecs,(long)last_nsecs);
            ttime=time_now.tv_sec;
            nsecs=time_now.tv_sec*1E9+time_now.tv_nsec;
            use_gps_event_time=0;
          } else {
      ...
      /* 
      *  GPS card is not locked or configured and is 
      *  reporting software time instead of event capture
      */      
      ttime=time_now.tv_sec;
      nsecs=time_now.tv_sec*1E9+time_now.tv_nsec;
      use_gps_event_time=0;
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.kod%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"KOD seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"KOD seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"KOD seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"KOD seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"KOD seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"KOD seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"KOD seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"KOD seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"KOD seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"KOD seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"KOD seq %d :: ACFCalculate xcf\n",nave);
        ...
        fprintf(stderr,"KOD seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"KOD seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"KOD SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"KOD SiteIntegrate: end: nave: %d\n",nave);
   }
