...
void SiteKsrExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteKsrExit: Sig %d: %d\n",signum,KSR_exit_flag); 
      cancel_count++;
      KSR_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteKsrExit: Sig %d: %d\n",signum,KSR_exit_flag); 
...
int SiteKsrStart(char *host) {
  signal(SIGPIPE,SiteKsrExit);
  signal(SIGINT,SiteKsrExit);
  signal(SIGUSR1,SiteKsrExit);
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteKsrStart: rxchn=%d\n",rxchn);

  } 

/* Adjust start of daytime and nighttime operations */
  day=14;
  night=2;
...
int SiteKsrSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Ksr_%d_%d",rnum,cnum);
...
/* Setup the seqlog file here*/
  ...
  if(msglog_dir!=NULL) { 
    ...
    sprintf(msglog_name,"%s/msglog.ksr%s.%04d%02d%02d",msglog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
  ...
  if(seqlog_dir!=NULL) { 
    ...
    sprintf(seqlog_name,"%s/seqlog.ksr%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
int SiteKsrStartIntt(int sec,int usec) {
  ...
  SiteKsrExit(0);
  if (debug) {
    fprintf(stderr,"SiteKsrStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteKsrStartInt: end\n");
  }
  return 0;
...
int SiteKsrIntegrate(int (*lags)[2]) {
  ...
  uint32 uQ32,uI32;
  if (debug) {
    fprintf(stderr,"KSR SiteIntegrate: start\n");
  }
  SiteKsrExit(0);
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"KSR GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"KSR GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"KSR GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"KSR GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"KSR Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"KSR nave=%d\n",nave);
      fprintf(stderr,"KSR dprm.status=%d\n",dprm.status);
    }
    ...
    gmtime_r(&ttime,&tstruct);
    if(seqlog_dir!=NULL) { 
      sprintf(filename,"%s/seqlog.ksr%s.%04d%02d%02d",seqlog_dir,channame,tstruct.tm_year+1900,tstruct.tm_mon+1,tstruct.tm_mday);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"KSR seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"KSR seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"KSR seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"KSR seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"KSR seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"KSR seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"KSR seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"KSR seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"KSR seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"KSR seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"KSR seq %d :: ACFCalculate xcf\n",nave);
          ACFCalculate(&tsgprm,(int16 *) dest,rngoff,skpnum!=0,
        ...
        fprintf(stderr,"KSR seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"KSR seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"KSR SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"KSR SiteIntegrate: end: nave: %d\n",nave);
   }
