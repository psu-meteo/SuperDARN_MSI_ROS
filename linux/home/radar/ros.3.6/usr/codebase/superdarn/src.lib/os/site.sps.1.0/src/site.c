...
void SiteSpsExit(int signum) {
  ...
  switch(signum) {
    case 2:
      if (debug) printf("SiteSpsExit: Sig %d: %d\n",signum,SPS_exit_flag); 
      cancel_count++;
      SPS_exit_flag=signum;
      if (cancel_count < 3 )
        break;
    case 0:
      if (debug) printf("SiteSpsExit: Sig %d: %d\n",signum,SPS_exit_flag); 
...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteSpsStart: rxchn=%d\n",rxchn);
...
int SiteSpsSetupRadar() {
  ...
  sprintf(sharedmemory,"IQBuff_Sps_%d_%d",rnum,cnum);
...
int SiteSpsStartIntt(int sec,int usec) {
  ...
  SiteSpsExit(0);
  if (debug) {
    fprintf(stderr,"SiteSpsStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteSpsStartInt: end\n");
  }
  return 0;
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"SPS GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"SPS GET_DATA:status=%d\n",rmsg.status);
    }
    smsg.type=GET_PARAMETERS;
    TCPIPMsgSend(sock, &smsg, sizeof(struct ROSMsg));
    TCPIPMsgRecv(sock, &rprm, sizeof(struct ControlPRM));
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"SPS GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"SPS GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"SPS Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"SPS nave=%d\n",nave);
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"SPS seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"SPS seq %d :: rdata.main 16bit :\n",nave);
        ...
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"SPS seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.back)[8],((int16 *)rdata.back)[9],
           ((int16 *)rdata.back)[10],((int16 *)rdata.back)[11]);
        dest += dprm.samples*4;
        fprintf(stderr,"SPS seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"SPS seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        fprintf(stderr,"SPS seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"SPS seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"SPS SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",(long int)iqsze,(long int)iqsze/2,(long int)iqsze/4);
     fprintf(stderr,"SPS SiteIntegrate: end: nave: %d\n",nave);
   }
